...
def normalize_answer(s):
    ...
    def remove_punc(text):
        exclude = set(list(string.punctuation) + punc)
...
def get_tokens(in_str, rm_punc=False):
    in_str = str(in_str).lower().strip()
    segs_out = []
    temp_str = ""
    for char in in_str:
        if rm_punc and char in punc:
            continue
        if re.search(u'[\u4e00-\u9fa5]', char) or char in punc:
            if temp_str != "":
...
def compute_f1(a_gold, a_pred):
    gold_toks = get_tokens(a_gold, True)
    pred_toks = get_tokens(a_pred, True)
    _, lcs_len = find_lcs(gold_toks, pred_toks)
    if lcs_len == 0:
        return 0
    precision = 1.0 * lcs_len / len(pred_toks)
    recall = 1.0 * lcs_len / len(gold_toks)
